...
const Cart = () => {
    ...
    const totalQty = safeData.reduce((prevValue,currValue)=> prevValue + (currValue.quantity || 0), 0)
    const totalPrice = safeData.reduce((prevValue,currValue)=> prevValue + ((currValue.quantity || 0) * (currValue?.productId?.sellingPrice || 0)),0)
    ...
    return (
        <div className='container mx-auto'>
            <div className='text-center text-lg my-3'>
                {
                    error && (
                        <p className='bg-red-100 text-red-600 py-2 px-4 rounded'>{error}</p>
                    )
                }
                {
                                    ...
                                    <div key={product?._id+"Add To Cart Loading"} className='w-full bg-white h-32 my-2 border border-slate-300 rounded flex relative'>
                                        ...
                                        <div className='px-4 py-2'>
                                            ...
                                            <div className='flex items-center justify-between'>
                                                ...
                                                <p className='text-slate-600 font-semibold text-lg absolute right-0 p-2'>{displayPKRCurrency((product?.productId?.sellingPrice || 0) * (product?.quantity || 0))}</p>
                                            </div>
